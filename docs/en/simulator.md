...
- **Exit fallback**: a `process.on('exit')` handler does a best-effort synchronous `SIGKILL` sweep of any remaining sessions, so an abrupt host exit does not leave orphaned simulator windows or a held debug port.
...
128 cases across 3 test files (grouped into 6 areas). A fresh clone must run `npm run build:simulator:runtime` first — most groups depend on build artifacts that are not in version control.
...
The `prepareResources` end-to-end test points `runtimeRoot` at a temp dir and only mocks the three dependencies that need a real project / preview server; everything else runs against the real native executable and engine artifacts.
