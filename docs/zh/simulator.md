...
- **退出兜底**：`process.on('exit')` 会对残留会话做**同步** best-effort 的 `SIGKILL` 清理，宿主被突然杀掉时不会留下孤儿 simulator 窗口或被占住的调试端口。
...
128 个用例，3 个测试文件（按关注点分 6 组）。新克隆的仓库必须先跑 `npm run build:simulator:runtime` —— 多数组依赖构建产物，而产物不在版本库里。
...
`prepareResources` 的端到端用例用 `runtimeRoot` 入参把产物写进临时目录，只 mock 掉需要真实项目 / preview server 的三个依赖，其余全走真实 native 可执行文件和引擎产物。
