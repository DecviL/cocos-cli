...
describe('PreviewPlayService (game view 状态机)', () => {
    ...
    let inputStub: {
        _frameDispatchEvents: jest.Mock | ((...args: any[]) => void);
        _clearEvents: jest.Mock;
        _dispatchMouseDownEvent: jest.Mock;
        _dispatchMouseMoveEvent: jest.Mock;
        _dispatchMouseUpEvent: jest.Mock;
        _dispatchMouseScrollEvent: jest.Mock;
        _dispatchKeyboardDownEvent: jest.Mock;
        _dispatchKeyboardUpEvent: jest.Mock;
    };
    ...
    it('play 态：画布输入转发到 cc.input 并短路编辑器处理（EDITOR 构建无原生 DOM 监听）', async () => {
        await service.start(SCENE_JSON);
        ...
        const evt = { clientX: 10, clientY: 20, button: 0, buttons: 1 };
        // 门控必须短路编辑器侧处理（返回 false）；dispatch 目标为引擎 input 实例（模块导入），
        // 其 wiring 由运行时验收覆盖（播放态按钮点击）。
